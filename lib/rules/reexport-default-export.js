    ...
    create(context) {
        ...
        return {
            ...
            ExportDefaultDeclaration: (node) => {
                // Avoid test/style/schema files
                const filename = path.parse(context.getPhysicalFilename()).name;
                if (/\./.test(filename) || path.parse(path.dirname(context.getPhysicalFilename()) ).name === 'src') return;
